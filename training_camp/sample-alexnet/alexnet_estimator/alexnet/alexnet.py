...
def dropout(x, chip, is_training):
    if is_training:
        if chip == 'npu':
            x = npu_ops.dropout(x, 0.65)
        elif chip in ['gpu', 'cpu']:
            x = tf.nn.dropout(x, 0.65)
    else:
        if chip == 'npu':
            x = npu_ops.dropout(x, 1.0)
        elif chip in ['gpu', 'cpu']:
            x = tf.nn.dropout(x, 1.0)

    return x

def inference_alexnet_impl(inputs, num_classes=1000, is_training=True, chip='npu'):
...
def inference_alexnet_impl_he_uniform(inputs,num_classes=1000, is_training=True, chip='npu'):
...
def inference_alexnet_impl_he_uniform_custom(inputs,num_classes=1000, is_training=True, chip='npu'):
    '''
...
def inference(inputs,version="xavier",num_classes=1000, is_training=False, chip='npu'):

    if chip == 'npu':
        from npu_bridge.estimator import npu_ops 

    if version=='xavier':
        return  inference_alexnet_impl(inputs,num_classes, is_training=is_training, chip=chip)
    elif version=='he_uniform':
        return  inference_alexnet_impl_he_uniform(inputs,num_classes, is_training=is_training, chip=chip)
    elif version=='he_uniform_custom':
        return inference_alexnet_impl_he_uniform_custom(inputs, num_classes,is_training=is_training, chip=chip)
    else:
