...
class ImageData:
    ...
    def image_processing(self, filename):
        ...
        if not self.custom_dataset :
            ...
            # print("image_processing: filename=",filename)
...
def load_data(dataset_name) :
    if dataset_name == 'mnist' :
        ...
        print("glob() is ::---------->",os.path.join("./dataset", dataset_name, '*.*'))
        # x = glob(os.path.join("./dataset", dataset_name, '*.*'))
        x = glob("/cache/user-job-dir/workspace/device0/dataset/CAT_00/*.jpg")
        # x = glob(os.path.join("./dataset/*.*"))
