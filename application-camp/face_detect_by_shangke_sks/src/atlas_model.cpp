...
:loadFlag_(false), modelId_(0), modelMemPtr_(nullptr),
...
AtlasModel::AtlasModel(const std::vector<uint8_t>& modelData)
:modelData_(modelData), loadFlag_(false), modelId_(0), modelMemPtr_(nullptr),
modelMemSize_(0),modelWeightPtr_(nullptr),modelWeightSize_(0),
modelDesc_(nullptr), input_(nullptr), output_(nullptr), outputsNum_(0),
isReleased_(false) {
}
...
AtlasError AtlasModel::Init() {
    ...
    AtlasError ret = LoadModelFromMem(modelData_);
    if (ret != ATLAS_OK) {
    ...
    ATLAS_LOG_INFO("Init model success");

    return ATLAS_OK;
}

AtlasError AtlasModel::Init(const std::vector<uint8_t>& modelData) {
    modelData_ = modelData;
    return Init();
}

AtlasError AtlasModel::LoadModelFromMem(const std::vector<uint8_t>& modelData) {
    if (loadFlag_) {
        ATLAS_LOG_ERROR("model is loaded already");
        return ATLAS_ERROR_LOAD_MODEL_REPEATED;
    }

    aclError ret = aclmdlLoadFromMem(modelData.data(), modelData.size(), &modelId_);
    if (ret != ACL_ERROR_NONE) {
        ATLAS_LOG_ERROR("Load model from file return %d", ret);
        return ATLAS_ERROR_LOAD_MODEL;
    }

    loadFlag_ = true;
    ATLAS_LOG_INFO("Load model success");
...
AtlasError AtlasModel::CreateDesc() {
    ...
    if (ret != ACL_ERROR_NONE) {
        ATLAS_LOG_ERROR("Get model description failed");
...
{
    if (modelDesc_ == nullptr) {
        ATLAS_LOG_ERROR("Create output failed for no model description");
...
AtlasError AtlasModel::Execute(vector<InferenceOutput>& inferOutputs) {
    ...
    if (ret != ACL_ERROR_NONE) {
        ATLAS_LOG_ERROR("Execute model error:%d", ret);
...
{
    if (!loadFlag_) {
        ATLAS_LOG_INFO("Model had not been loaded or unload already");
        return;
    }

    aclError ret = aclmdlUnload(modelId_);
    if (ret != ACL_ERROR_NONE) {
        ATLAS_LOG_ERROR("Unload model error:%d", ret);
    }
    ...
    loadFlag_ = false;
    ATLAS_LOG_INFO("Unload model success");
}
