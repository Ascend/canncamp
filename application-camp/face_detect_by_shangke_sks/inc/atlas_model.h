...
public:
    AtlasModel();
    AtlasModel(const std::vector<uint8_t>& modelData);

    ~AtlasModel();

    AtlasError Init();
    AtlasError Init(const std::vector<uint8_t>& modelData);
...
private:
    AtlasError LoadModelFromMem(const std::vector<uint8_t>& modelData);
