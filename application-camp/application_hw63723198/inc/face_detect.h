...
class FaceDetect {
public:
    FaceDetect(aclrtRunMode runMode);
    ~FaceDetect();

    AtlasError Init();
    void DestroyResource();
    AtlasError Process(cv::Mat& frame, vector<DetectionResult>& detectResults);
private:
    AtlasError create_input();
    AtlasError PreProcess(cv::Mat& frame);
    AtlasError Inference(vector<InferenceOutput>& inferOutputs);
                                 
    void PostProcess(vector<DetectionResult>& detectResults,
